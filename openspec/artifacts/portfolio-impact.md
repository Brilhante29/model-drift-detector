# Portfolio Impact: model-drift-detector

## Program

- Program id: `mlops-data-platform`
- Program name: MLOps and Data Platform
- Component pack: `mlops-data-platform`

## System Story

Guards the model lifecycle from #21 after deployment by turning immutable feature and prediction batches into auditable drift alarms and labeled benchmark evidence.

This repository is not a standalone demo. It is one part of the MLOps and Data Platform system and should produce reusable fixtures, benchmark patterns, and decisions for later repositories.

## Proficiency Signal

- Primary profile: `python`
- Stack profile: `python-ml`
- Stack:
- python-3.12
- numpy-2.5.1
- scipy-1.18.0
- pydantic-2.13.4
- prometheus-client-0.25.0
- docker

## Post Angle

Open with drift_alarm_f1 = pending, then explain why the architecture and local-first path make the result reproducible.
